  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed inset-0 z-[60] flex flex-col bg-brand text-white transition-opacity duration-300 ${
          open ? "opacity-100" : "pointer-events-none opacity-0"
        }`}
      >
        <div className="mx-auto flex h-16 w-full max-w-6xl items-center justify-end px-5 md:h-20 md:px-8">
          <button
            type="button"
        ...
        <nav className="flex flex-1 flex-col items-center justify-center gap-2">
          {nav.map((item, i) => (
            <a
              key={item.href}
              href={item.href}
              className={`px-4 py-2 text-4xl font-extrabold tracking-tight text-white transition-all duration-300 hover:text-white/70 md:text-5xl ${
