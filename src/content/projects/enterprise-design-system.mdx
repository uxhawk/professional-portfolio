...
> **[PLACEHOLDER — problem evidence]** *Add a data point that made the pain concrete: e.g., support tickets tagged navigation/orientation, a customer quote from a multi-product account, or time-to-proficiency differences between single- and multi-product customers.*
...
> **[PLACEHOLDER — research sessions]** *If you ran user-facing research here (customer interviews, navigation studies, tree tests on the IA, usability sessions on the shell), name the sessions, participants, and counts — Suzy specifically looks for what the sessions looked like.*

## Flows

> **[FLOW DIAGRAM PLACEHOLDER]** — *Spec: one diagram of the cross-application way-finding journey — a multi-product user entering the platform, orienting via the shared shell (primary nav → secondary nav → breadcrumbs → page heading), then switching products through the application switcher and landing already-oriented because every zone is in the same place. Annotate the moments the contracts protect: where navigation lives, where state surfaces, and where the switcher preserves context. This diagram doubles as the visual argument for the whole system.*
...
> **[PLACEHOLDER — honest pivot]** *Add one genuine failure or reversal: a contract that was wrong on the first pass, an application whose adoption stalled and why, or a pattern you had to walk back after implementation. Confirm or replace the density-modes story above — the pushback framing is inferred from the shipped spec, not documented history.*
...
> **[PLACEHOLDER — business KPIs]** *Add 1–2 numbers with baselines: e.g., reduction in design/build time for new product onboarding, support ticket reduction for navigation issues, or multi-product customer satisfaction/retention movement.*
...
> **[PLACEHOLDER — user KPIs]** *Add evidence users felt the change: task success or time-on-task deltas, SUS/NPS movement, or a customer quote from a multi-product account.*
...
**[PLACEHOLDER — verify this reflection matches reality, or replace with your own]** The contracts were validated against the state matrix, but adoption was measured in applications shipped, not in user outcomes — I can say all 17 products adopted the shell, and I can’t say precisely how much faster a multi-product user orients today than before. Given another pass, I’d define the user-facing success metric (cross-application task success, orientation time on first entry) before the first audit, and benchmark two or three applications pre-adoption so the system’s impact would be a measured claim rather than a structural one.
