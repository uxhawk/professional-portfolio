...
The authoring loop in one view. Shaded stages run automated; highlighted stages are the deliberate human decision points — and note that AI generation runs only when the author asks, with the instruct-and-regenerate path looping reviewed content back through generation on the author’s terms.

![Flow diagram of the authoring loop: open case → compose from template → AI section generation → review against sources → accept, edit, regenerate → assemble document → send to carrier — with an instruct-and-regenerate loop back into generation, and every block declaring its origin as AI, edited by human, or final.](/images/2026/08/ai-authoring-flow.png)
